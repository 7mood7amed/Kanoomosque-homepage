...
export function Stats() {
  return (
    <section id="stats" className="relative w-full overflow-hidden rounded-lg border border-tertiary-fixed-dim/20 bg-surface-container-lowest/85 py-20 shadow-[0_25px_60px_-30px_rgba(24,29,21,0.35)] backdrop-blur-2xl sm:py-28">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <motion.div
            ...
            className="rounded-lg border border-outline-variant bg-surface-container px-6 pb-6 pt-8 text-center shadow-sm transition-shadow hover:shadow-md">

              <p className="font-display-lg text-4xl leading-[1.5] text-on-surface">
                {stat.value === '—' ?
              <span className="text-2xl text-on-surface-variant">مقرئون</span> :

              stat.value
              }
              </p>
              <p className="mt-2 text-body-md font-bold text-on-surface">{stat.label}</p>
              <p className="mt-1 text-caption leading-6 text-on-surface-variant">{stat.caption}</p>
            </motion.div>
          )}
        </div>
        ...
        <motion.div
          ...
          className="frame-ornate-dark relative mt-8 overflow-hidden rounded-3xl bg-inverse-surface p-3 shadow-2xl">
